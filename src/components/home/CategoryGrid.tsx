...
const categories = [
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1584036561566-b452744e4950?q=80&w=2070&auto=format&fit=crop',
    color: 'bg-kerbl-green',
    href: '#',
    colSpan: 'col-span-1',
  },
  ...
  },
  {
    title: 'Fencing',
    image:
      'https://images.unsplash.com/photo-1599156627038-d621b02014de?q=80&w=2069&auto=format&fit=crop',
    color: 'bg-kerbl-green',
    href: '#',
    colSpan: 'col-span-1',
  },
...
export function CategoryGrid() {
  return (
    <section className='container mx-auto px-4 mt-8 relative z-30 mb-12'>
      <StaggerContainer
      ...
      >
        {categories.map((category, index) => (
          <SlideInLeft
            key={index}
            className={cn(
              'group relative block overflow-hidden shadow-lg h-full',
              category.colSpan
            )}
            delay={index * 0.1}
          >
            <Link href={category.href} className='block w-full h-full'>
              <div className='aspect-[4/3] w-full relative'>
                ...
                <div
                  className={cn(
                    'absolute top-2 left-2 px-2 py-1 text-white text-xs font-bold uppercase',
                    category.color
                  )}
                >
                  {category.title}
              ...
              <div className='absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity hidden lg:flex items-center justify-center'>
                <span className='text-white font-bold border-2 border-white px-4 py-2 uppercase tracking-wider'>
                  Explore
                </span>
              </div>
            </Link>
